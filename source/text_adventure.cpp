...
TokenizeCommand(memory_arena *Memory, const char *Command){
    ...
    while(CurrentIndex < CommandLength){
        ...
        if(WordLength > 0){
            char *Token = ArenaPushArray(Memory, char, WordLength+1);
...
CompareWordsPercentage(const char *A, const char *B){
    ...
    s32 *V0 = ArenaPushArray(&GlobalTransientMemory, s32, N+1);
    s32 *V1 = ArenaPushArray(&GlobalTransientMemory, s32, N+1);
...
internal inline ta_found_item *
TAContinueFindName(ta_found_items *Founds, ta_name_comparison *Comparison){
...
TAContinueFindItems(ta_system *TA, array<ta_id> *Items, word_array Words, ta_found_items *Founds){
    for(u32 ItemIndex=0; ItemIndex<Items->Count; ItemIndex++){
        ...
        ta_found_item *Found = TAContinueFindName(Founds, &Comparison);
        if(Found){
            Found->Item = Item;
...
internal inline b8
TAItemIsAnEater(ta_system *TA, ta_item *Item){
    ta_data *Feed = TA->FindDescription(&Item->Datas, AssetTag(AssetTag_Feed));
    return (Feed != 0);
}
...
ta_system::AllocEditingCommand(){
    ta_editing_command_node *Node = ArenaPushType(&GlobalTickMemory, ta_editing_command_node);
